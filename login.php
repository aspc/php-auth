...
elseif (isset($_COOKIE[$DJANGO_SESSION_COOKIE_NAME])) {
	...
	try {
		...
		mail("digitalmedia@aspc.pomona.edu", "[ASPC] PHP ERROR: login.php:41", (string)$e->getMessage());
		die();
	}
	...
	if (is_object($result) && property_exists($result, "error")) {
		...
		mail("digitalmedia@aspc.pomona.edu", "[ASPC] PHP ERROR: login.php:48", (string)$result->error);
	...
	elseif (is_array($result)) {
		...
		if (strpos($username_parts[1], "pomona.edu") !== FALSE) {
			...
			try {
				...
				mail("digitalmedia@aspc.pomona.edu", "[ASPC] PHP ERROR: login.php:67", (string)$e->getMessage());
				header("Location: https://aspc.pomona.edu/accounts/login/");
				die();
			}
		}
	...
	# Something unexpected happened... Don't know how to handle, so just try logging in again
		...
		mail("digitalmedia@aspc.pomona.edu", "[ASPC] PHP ERROR: login.php:87", (string)var_dump($result));
		header("Location: https://aspc.pomona.edu/accounts/login/");
		die();
	}
}
